...
// 2. CONFIRMAÇÃO E GRAVAÇÃO COM ASTROLOGIA REAL
async function confirmAndProceed() {
    const btn = document.querySelector('#confirmation-modal .btn-primary');
    const originalText = btn.innerText;
    btn.innerText = "CONSULTANDO OS ASTROS...";
    btn.disabled = true;

    try {
        // Chamada para a sua nova API na Vercel que configuraremos abaixo
        ...
        const mapaReal = await response.json();

        if (mapaReal.error) throw new Error(mapaReal.error);
        ...
        const finalUserData = {
            ...tempUserData,
            ...numerologia,
            mapaAstral: mapaReal, // O Mapa real da AstrologyAPI agora vive aqui!
        ...
        guruAlert("As brumas estelares estão densas. Verifique sua conexão ou dados de nascimento.");
        console.error("Erro no Batismo:", error);
    } finally {
